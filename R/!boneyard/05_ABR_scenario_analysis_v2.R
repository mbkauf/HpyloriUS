get_sis_abr_test_treat <- function(p_trt_eff = NULL,
  ...
  print(v_parameter$v_psi)
  # Scenario 2: 18+, low mutation rate (0.04)
  v_parameter <- load_sis_abr_model_params_all(
    v_race = c("Hispanic", "NH Black", "NH White", "Other"),
    waifw = m_waifw,
    trt_year = 1,
    ...
    v_init_state = v_burn_state,
    ages = groups)
  v_parameter$test_sens <- 0.95
  v_parameter$age_based_test <- FALSE
  v_parameter$sigma <- 0.04
  v_parameter$v_psi <- 1
  ...
  print(v_parameter$v_psi)
  # Scenario 3: 18+, high mutation rate (0.71)
  v_parameter <- load_sis_abr_model_params_all(
    v_race = c("Hispanic", "NH Black", "NH White", "Other"),
    waifw = m_waifw,
    trt_year = 1,
  ...
  df_all_long <- rbind(m_hisp, m_black, m_white, m_other) %>%
    ...
    dplyr::mutate(scenario = factor(scenario, levels = c(1, 2, 3),
                                    labels = c("No Treatment",
                                               "18+: Low Mutation Rate",
                                               "18+: High Mutation Rate")))
...
ggsave(plot = plot_results_grid_assort,
       filename = "results/abr_tnt_strain_assort_v2.pdf",
       width = 10, height = 8)
...
ggsave(plot = plot_results_grid_rp,
       filename = "results/abr_tnt_strain_rp_v2.pdf",
       width = 10, height = 8)
...
ggsave(plot = plot_results_all,
       filename = "results/abr_tnt_strain_rp_assort_v2.pdf",
       width = 12, height = 6)
...
ggsave(plot = pct_plots_abr[[1]], filename = "results/abr_assort_rp_net_v2.pdf",
       height = 8, width = 12)
ggsave(plot = pct_plots_abr[[2]], filename = "results/abr_assort_rp_rel_v2.pdf",
       height = 8, width = 12)
